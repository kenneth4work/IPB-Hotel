...
import { Info, Camera, Plus } from 'lucide-react';
...
export function RoomCard({ room, onOpenGallery, onOpenDetail }: RoomCardProps) {
  ...
  // Handler: Ganti foto pada slide aktif saat ini
  const handleReplacePhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImagesList((prev) => {
      const updated = [...prev];
      updated[currentIdx] = objectUrl;
      return updated;
    });

    // Reset input value agar dapat memilih file yang sama jika diinginkan
    e.target.value = '';
  };

  // Handler: Tambah foto baru ke carousel kamar
  const handleAddPhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImagesList((prev) => {
      ...
      // Arahkan slide langsung ke foto baru yang baru ditambahkan
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-slate-800 text-[11px] font-bold px-3 py-1 rounded-md uppercase tracking-wider shadow-sm border border-white/40 pointer-events-none">
          {room.badge}
        </span>
        <span 
